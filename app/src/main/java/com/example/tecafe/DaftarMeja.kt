                ...
                * The DataSnapshot object represents a particular location's data in the Firebase Realtime Database at the time of the event. */
                if (snapshot.exists()) {  // Snapshot means Data
                    ...
                    * By using the for loop with mejaSnap as the loop variable and snapshot.children as the iterable, you can iterate over each child node under the specified snapshot object.*/
                    for (mejaSnap in snapshot.children) { // mengambil data dari Menu child
                        ...
                        val mejaData =
                            mejaSnap.getValue(MejaModel::class.java) // referencing class MenuModel untuk mendapat semua data dari kelas tersebut
                        ...
                        mejaList.add(mejaData!!) // !! untuk menghindari null data
                    }
                    ...
                    mAdapter.setOnItemClickListener(object : MejaAdapter.onItemClickListener {
                        override fun onItemClick(position: Int) {
                            val intent = Intent(this@DaftarMeja, MejaDetails::class.java)

                            // put extras
                            intent.putExtra("mejaId", mejaList[position].mejaId)
                            intent.putExtra("mejaName", mejaList[position].mejaName)
                            intent.putExtra("mejaPrice", mejaList[position].mejaPrice)
                            intent.putExtra("mejaDesc", mejaList[position].mejaDesc)
